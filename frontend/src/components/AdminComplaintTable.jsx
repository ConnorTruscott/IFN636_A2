...
export default function AdminComplaintTable({
  ...
  const TH = ({ id, children }) => (
    <th
      ...
      style={{
        textAlign:'left', borderBottom:'1px solid #eee', padding:8,
        userSelect:'none', cursor:'pointer', whiteSpace:'nowrap'
      }}
    >
      <span style={{ fontWeight:600 }}>{children}</span>
      <span style={{ marginLeft:6, opacity: activeSort===id ? 1 : 0.3 }}>⇅</span>
    </th>
  );
  ...
  const TD = ({ children }) => (
    <td
      style={{
        padding:8, maxWidth:220, overflow:'hidden',
        ...
        <tbody>
          {complaints.map(c=>(
            <tr key={c._id} style={{ borderBottom:'1px solid #f0f0f0' }}>
              ...
              <TD>{c.assignedStaff?.fullname || '-'}</TD>
          ...
          {!complaints.length && (
            <tr><td colSpan="8" style={{ padding:12, color:'#777' }}>No complaints found.</td></tr>
          )}
        </tbody>
      </table>
    </div>
